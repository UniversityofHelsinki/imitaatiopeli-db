...
FROM (
         SELECT
             ...
             CASE
                 WHEN jg.answer_id IS NULL THEN NULL
                 WHEN jg.answer_id = a_non.answer_id THEN 1
                 ELSE 0
                 END AS correct,
             jg.confidence,
         ...
         FROM question q
                  JOIN player_combination pc
                       ON pc.game_id = q.game_id
                           AND pc.judge_id = q.judge_id
                  JOIN player pj
                       ON pj.player_id = q.judge_id
                  JOIN player pp
                       ON pp.player_id = pc.player_id
                           AND pp.is_pretender = FALSE
                  LEFT JOIN answer a_pretender
                            ON a_pretender.question_id = q.question_id
                                AND a_pretender.is_pretender = TRUE
                  LEFT JOIN answer a_non
                            ON a_non.question_id = q.question_id
                                AND a_non.player_id = pp.player_id
                                AND a_non.is_pretender = FALSE
                  LEFT JOIN judge_guess jg
                            ON jg.question_id = q.question_id
         ...
         FROM judge_final_guess jfg
                  JOIN player_combination pc
                       ON pc.game_id = jfg.game_id
                           AND pc.judge_id = jfg.judge_id
                  JOIN player pj
                       ON pj.player_id = jfg.judge_id
                  JOIN player pp
                       ON pp.player_id = pc.player_id
                           AND pp.is_pretender = FALSE
         WHERE jfg.game_id = $1
