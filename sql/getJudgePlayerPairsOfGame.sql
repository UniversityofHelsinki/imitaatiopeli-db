SELECT
    ...
    COALESCE(judge_question_counts.question_count, 0) AS judge_question_count,
...
FROM player_combination pc
    ...
    GROUP BY player_id, game_id
                   ...
                   ON answer_counts.player_id = pp.player_id
                       AND answer_counts.game_id = pc.game_id
         LEFT JOIN (
    SELECT judge_id, game_id, COUNT(*) AS question_count
    FROM question
    WHERE game_id = $1
    GROUP BY judge_id, game_id
) judge_question_counts
                   ON judge_question_counts.judge_id = pj.player_id
                       AND judge_question_counts.game_id = pc.game_id
         LEFT JOIN judge_final_guess jfg
                   ON jfg.game_id = pc.game_id
                       AND jfg.judge_id = pj.player_id
